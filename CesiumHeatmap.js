...
/*  Create a CesiumHeatmap instance
	...
	*/
CesiumHeatmap.create = function (cesium, bb, polygonObj, options) {
	var instance = new CHInstance(cesium, bb, polygonObj, options);
	return instance;
};
 ...
 */
function CHInstance(c, bb, polygonObj, o) {
	if (!bb) {
		return null;
	}
	if (!o) {
	...
	this._heatmap = heat.create(this._options);
...
CHInstance.prototype.updateLayer = function () {
	...
	if (CesiumHeatmap.defaults.useEntitiesIfAvailable && this._cesium.entities) {
		...
		var material = new Cesium.ImageMaterialProperty({ image: this._heatmap._renderer.canvas });
		...
		this._layer ? (this._layer.polygon.hierarchy = this._polygonObj, this._layer.polygon.material = material) : (this._layer = this._cesium.entities.add({
			show: true,
			polygon: {
				hierarchy: this._polygonObj,
