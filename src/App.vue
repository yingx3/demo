<template>
  <div class="top-container">
    ...
    ></zy-ml>
    <div v-if="currentAlert" :class="['alert', { 'alert-warning': currentAlert.isWarning }]">
    <h3>{{ currentAlert.name }}</h3>
    <p>当前值: {{ currentAlert.value }}</p>
  </div>
  </div>
</template>
<script setup>
...
const hd = ref('dangerLevel_20250105_223219_295_10800.png')
...
const addLayer4 = () => {
  Cesium.GeoJsonDataSource.load('/ng/hpps2.geojson').then(function (
    ...
    for (var i = 0; i < entities.length; i++) {
      ...
      entity.billboard = {
        // image: Cesium.Resource.createIfNeeded("http://127.0.0.1:8086/RI/pointRED1.png"), // 使用 Cesium.Resource 确保路径正确
        image: "/cs/img/pointRED1.png", 
        width: 30, // 控制图片宽度
        height: 30, // 控制图片高度
        scale: 1.5, // 图片缩放比例
...
const devicePositions = {
  "dev-001": {
    longitude: 87.77241, // 经度
    latitude: 27.92783,   // 纬度
    height: 0            // 高程
  },
  "dev-002": {
    longitude: 87.75662,
...
// 添加全局变量存储MQTT连接和实体映射
let mqttClient = null
const entityMap = new Map() // 存储设备ID对应的实体

// 在 setup() 中添加调试状态
const mqttStatus = ref('未连接')

const addLayer16 = () => {
  console.log('尝试连接 MQTT...')
  mqttStatus.value = '连接中'
  ...
  mqttClient.connect({
    onSuccess: () => {
      ...
      mqttClient.subscribe("sensors/water-level")
      
      // 发送测试消息验证
      // const message = new Paho.Message(
      //   JSON.stringify({ ID: "dev-001", "type":"Water level gauge", value: 1.8 })
      // )
      // message.destinationName = "sensors/water-level"
  ...
  mqttClient.onMessageArrived = (message) => {
    ...
    try {
      ...
      console.log('设备数据:', payload)
      if (!payload.ID) throw new Error("无效的 ID 字段")
      updateOrCreateEntity(payload)
    } catch (e) {
...
const updateOrCreateEntity = (payload) => {
  const { ID, value } = payload
  console.log('处理设备:', ID)
  
  const position = devicePositions[ID]
  if (!position) {
    console.warn(`未知设备坐标: ${ID}`)
    return
  }

  let entity = entityMap.get(ID)
  if (!entity) {
    console.log('创建新实体:', ID)
    entity = viewer.value.entities.add({
      ...
      billboard: {
        image: Cesium.buildModuleUrl('/cs/img/pointBLUE1.png'),
        width: 40,
        height: 40
      },
      label: {
        ...
        pixelOffset: new Cesium.Cartesian2(0, 40),  // Y轴偏移（正值为向上）
        verticalOrigin: Cesium.VerticalOrigin.BOTTOM // 标签底部对齐基准点
      }
    })
